 ...
 * File:   main.cpp
 * Author: Aamir
 * Created on February 2, 2020, 3:30 PM
 * Purpose:  Municipal Bond Savings V4 Using Dr.Mark's Savings Program
 *                        Using MVC w/ Functions
 */
...
void display(float balance[],int year[],int dYear[],int NYEARS,float intRate,float yrDpPc,float salary, float yrDep){
    //Display the Heading
...
void savings(float balance[],int year[],int dYear[],int NYEARS,float &intRate, float &yrDpPc,float &salary,float &yrDep){
    intRate/=CNVDCML;
    yrDep=yrDpPc*salary;
